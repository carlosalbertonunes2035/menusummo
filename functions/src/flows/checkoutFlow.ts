 ...
 */
export const secureCheckout = onCall(
    {
    ...
    async (request) => {
        ...
        if (!tenantId || !items || !Array.isArray(items)) {
            throw new HttpsError('invalid-argument', 'Dados inválidos: tenantId e items são obrigatórios.');
        }
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (!productDoc.exists) {
                    throw new HttpsError('not-found', `Produto não encontrado: ${productId}`);
                }
            ...
            throw new HttpsError('internal', `Falha ao processar checkout seguro: ${error.message}`);
        }
    }
);
