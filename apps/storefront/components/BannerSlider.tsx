...
export const BannerSlider: React.FC<BannerSliderProps> = ({
    ...
    return (
        <div className="relative w-full px-4 pt-2 pb-4 group">
            <div
                className="relative aspect-[2/1] w-full rounded-2xl overflow-hidden shadow-xl shadow-orange-600/10 border border-gray-200/50 bg-white cursor-pointer transform active:scale-[0.98] transition-all duration-300"
                ...
                {activeBanners.length > 1 && (
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5 z-10">
                        {activeBanners.map((_, idx) => (
                            <div
                                key={idx}
                                className={`h-1 rounded-full transition-all duration-500 ${idx === currentIndex ? 'w-4 bg-orange-600' : 'w-1.5 bg-white/40'}`}
                            />
                        ))}
                    </div>
                )}
