...
const getStatusInfo = (status: OrderStatus) => {
    switch (status) {
        ...
        case OrderStatus.DELIVERING: return { text: 'Em Rota', icon: Truck, color: 'text-purple-500 bg-purple-100' };
...
const OrdersTab: React.FC<OrdersTabProps> = ({ orders, user }) => {
    ...
    return (
        <div className="px-4 py-4 space-y-4 pb-24">
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">Meus Pedidos</h2>
            {myOrders.length === 0 ? (
                ...
                myOrders.map(order => {
                    ...
                    return (
                        <div key={order.id} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm animate-fade-in">
                            <div className="flex justify-between items-start mb-2">
                                <div>
                                    <span className="font-bold text-gray-800 dark:text-gray-100">Pedido #{order.id.slice(-4)}</span>
                                    <p className="text-xs text-gray-500 dark:text-gray-400">{new Date(order.createdAt).toLocaleDateString()} às {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                </div>
                                <span className={`px-2 py-1 rounded text-xs font-bold flex items-center gap-1 ${statusColor}`}>
                                    <StatusIcon size={12} /> {statusText}
                                </span>
                            </div>
                            <div className="border-t border-gray-50 dark:border-gray-700 pt-2 mt-2">
                                {order.items.map((item, i) => (
                                    <p key={i} className="text-sm text-gray-600 dark:text-gray-300">{item.quantity}x {item.productName}</p>
                                ))}
                            </div>
                            <div className="mt-3 pt-2 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                                <span className="text-sm font-bold text-gray-800 dark:text-gray-100">Total: R$ {order.total.toFixed(2)}</span>
