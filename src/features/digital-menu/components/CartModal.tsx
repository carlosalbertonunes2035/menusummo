...
const CartModal: React.FC<CartModalProps> = ({
    ...
    const renderHeader = () => (
        <div className="px-4 py-3 bg-white dark:bg-gray-900 sticky top-0 z-10 border-b border-gray-100 dark:border-gray-800">
            <div className="flex justify-between items-center mb-2">
                <button onClick={handleBack} className="p-2 -ml-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full transition">
                    <ChevronLeft size={24} />
                </button>
    ...
    const renderBagStep = () => (
        <div className={`flex-1 overflow-y-auto p-4 space-y-6 ${animationDirection === 'left' ? 'animate-slide-in-left' : 'animate-fade-in'}`}>
            <div className="flex items-center gap-3 pb-2">
                ...
                <div>
                    <h3 className="font-bold text-gray-800 dark:text-white leading-tight">{settings.brandName}</h3>
            ...
            {cart.length === 0 ? (
                ...
                <>
                    <div className="space-y-4">
                        <h4 className="font-bold text-gray-800 dark:text-white text-sm">Itens adicionados</h4>
                        {cart.map((item, idx) => {
                            ...
                            return (
                                <div key={idx} className="flex gap-4 bg-white dark:bg-gray-800 p-1 rounded-2xl group">
                                    <div className="w-16 h-16 bg-gray-100 dark:bg-gray-700 rounded-xl flex-shrink-0 overflow-hidden border border-gray-100 dark:border-gray-600">
                                        {channel.image && <img src={channel.image} className="w-full h-full object-cover" />}
                                    </div>
                                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <p className="font-bold text-sm text-gray-800 dark:text-gray-100 leading-tight">{channel.displayName || item.product.name}</p>
                                                <p className="text-[10px] text-gray-500 mt-0.5">{(price + optionsPrice).toFixed(2)} / un</p>
                                            </div>
                                            <p className="text-sm font-bold text-gray-900 dark:text-white">R$ {((price + optionsPrice) * item.quantity).toFixed(2)}</p>
                                        </div>
                                        {item.selectedOptions && item.selectedOptions.length > 0 && (
                                            <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 truncate">{item.selectedOptions.map(o => o.optionName).join(', ')}</p>
                                        )}
                                        {item.notes && <p className="text-[10px] text-orange-500 italic truncate">Obs: {item.notes}</p>}
                                    </div>
                                    <div className="flex items-center gap-3 bg-gray-50 dark:bg-gray-700 rounded-lg px-2 py-1 h-fit self-center border border-gray-200 dark:border-gray-600">
                    ...
                    {upsellItems.length > 0 && (
                        <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <span className="text-[10px] font-bold text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full uppercase tracking-widest">Sugestões</span>
                            </div>
                            ...
                            <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 -mx-4 px-4 scroll-smooth snap-x">
                                {upsellItems.map(prod => {
                                    ...
                                    return (
                                        <div key={prod.id} className="snap-start min-w-[150px] w-[150px] bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700/50 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col group active:scale-[0.98]">
                                            ...
                                            <div className="p-3 flex-1 flex flex-col">
                                                <p className="text-xs font-bold text-gray-800 dark:text-gray-100 leading-tight line-clamp-2 min-h-[32px]">{prod.name}</p>
                                                <div className="mt-2 flex items-baseline gap-1.5">
                                                    <span className="text-sm font-black text-green-600 dark:text-green-400">R$ {price.toFixed(2)}</span>
                                                    {hasPromo && (
                    ...
                    {appliedCoupon ? (
                        <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-xl border border-green-200 dark:border-green-800 flex items-center justify-between gap-3 animate-fade-in">
                            <div className="flex items-center gap-3">
                                <div className="bg-green-100 dark:bg-green-800 p-2 rounded-full text-green-600 dark:text-green-300"><TicketIcon size={20} /></div>
                                <div>
                                    <p className="font-bold text-green-800 dark:text-green-200 text-sm">Cupom Aplicado!</p>
                                    <p className="text-[10px] text-green-700 dark:text-green-300">{appliedCoupon.code} - Economia de R$ {discountValue.toFixed(2)}</p>
                                </div>
                            </div>
                        ...
                        <div className="space-y-3">
                            {coupons.length > 0 && (
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h4 className="font-bold text-gray-800 dark:text-white text-sm">Cupons Disponíveis</h4>
                                    </div>
                                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                                        {coupons.map(c => (
                                            <button
                                                ...
                                                className="border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/30 rounded-xl p-3 min-w-[140px] text-left hover:bg-green-100 dark:hover:bg-green-900/50 transition relative group active:scale-95 flex flex-col justify-between"
                                            >
                                                <div>
                                                    <p className="font-bold text-green-700 dark:text-green-400 text-xs mb-0.5">{c.code}</p>
                                                    <p className="text-[10px] text-green-600 dark:text-green-500 leading-tight">{(c as any).title || `${c.value}% OFF`}</p>
                                                </div>
                            ...
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-dashed border-gray-300 dark:border-gray-700 space-y-3">
                                <div className="flex items-center gap-3">
                                ...
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        className="flex-1 bg-white dark:bg-gray-700 px-3 py-2 rounded-lg text-xs font-mono uppercase border border-gray-200 dark:border-gray-600 outline-none focus:ring-1 focus:ring-summo-primary"
                                    />
                                    <button onClick={handleApplyCoupon} className="text-xs font-bold text-summo-primary bg-white dark:bg-gray-800 border border-summo-primary/20 px-4 py-2 rounded-lg shadow-sm hover:bg-summo-bg">
                                        Aplicar
                                    </button>
                                </div>
                            </div>
                    ...
                    <div className="space-y-2 pt-2">
                        <h4 className="font-bold text-gray-800 dark:text-white text-sm">Resumo de valores</h4>
                        <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400"><span>Subtotal</span><span>R$ {cartTotal.toFixed(2)}</span></div>
                        {appliedCoupon && (
                        ...
                        <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400"><span>Taxa de Entrega</span><span className={deliveryFee === 0 ? 'text-green-500 font-bold' : ''}>{isDelivery ? (deliveryFee === 0 ? 'Grátis' : `R$ ${deliveryFee.toFixed(2)}`) : 'N/A'}</span></div>
    ...
    const renderIdentityStep = () => (
        <div className={`flex-1 overflow-y-auto p-4 space-y-6 ${animationDirection === 'right' ? 'animate-slide-in-right' : 'animate-slide-in-left'}`}>
            <div className="text-center py-4">
                ...
                <h3 className="font-bold text-gray-800 dark:text-white text-xl">Vamos nos conhecer?</h3>
            ...
            <div className="space-y-4 bg-white dark:bg-gray-800 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <div><label className="text-xs font-bold text-gray-500 uppercase mb-2 block ml-1">Seu Nome</label><input type="text" value={identityName} onChange={e => setIdentityName(e.target.value)} className="w-full p-4 bg-gray-50 dark:bg-gray-700 dark:text-white border border-gray-200 dark:border-gray-600 rounded-xl outline-none focus:ring-2 focus:ring-summo-primary transition font-medium" placeholder="Como podemos te chamar?" autoFocus /></div>
                <div><label className="text-xs font-bold text-gray-500 uppercase mb-2 block ml-1">WhatsApp / Celular</label><div className="relative"><Smartphone className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={20} /><input type="tel" value={identityPhone} onChange={e => setIdentityPhone(e.target.value)} className="w-full pl-12 p-4 bg-gray-50 dark:bg-gray-700 dark:text-white border border-gray-200 dark:border-gray-600 rounded-xl outline-none focus:ring-2 focus:ring-summo-primary transition font-medium" placeholder="(00) 90000-0000" /></div><p className="text-[10px] text-gray-400 mt-2 ml-1 flex items-center gap-1"><AlertCircle size={10} /> Enviaremos o status do pedido por aqui.</p></div>
            </div>
        </div>
    );
    ...
    const renderFulfillmentStep = () => (
        <div className={`flex-1 overflow-y-auto p-4 space-y-6 ${animationDirection === 'right' ? 'animate-slide-in-right' : 'animate-slide-in-left'}`}>
            <h3 className="font-bold text-gray-800 dark:text-white text-lg">Como você quer receber?</h3>
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <div className="p-4">
                    <div className="flex justify-between items-start mb-4">
                        <div><p className="text-xs font-bold text-gray-400 uppercase mb-1">Entregar no endereço</p>{user.address ? (<><p className="font-bold text-gray-800 dark:text-white text-sm">{user.address.split(',')[0]}</p><p className="text-xs text-gray-500">{user.address}</p></>) : (<p className="text-sm text-gray-500 italic">Nenhum endereço selecionado</p>)}</div>
                    ...
                    <div className="bg-gray-100 dark:bg-gray-700 p-1 rounded-xl flex gap-1">
                        {[{ id: OrderType.DELIVERY, label: 'Entrega', icon: Bike }, { id: OrderType.TAKEOUT, label: 'Retirada', icon: Store }].map(m => (<button key={m.id} onClick={() => setOrderMode(m.id)} className={`flex-1 py-2.5 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition shadow-sm ${orderMode === m.id ? 'bg-white dark:bg-gray-600 text-summo-primary shadow' : 'text-gray-500 hover:bg-white/50'}`}><m.icon size={16} /> {m.label}</button>))}
                    </div>
                </div>
            </div>
            <div>
                <h4 className="font-bold text-gray-800 dark:text-white text-sm mb-3">Opções de {orderMode === OrderType.DELIVERY ? 'entrega' : 'retirada'}</h4>
                <div className="space-y-3">
                    <label className={`flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition ${!isScheduling ? 'border-summo-primary bg-summo-bg/20' : 'border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800'}`}><div className="flex items-center gap-3"><div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${!isScheduling ? 'border-summo-primary' : 'border-gray-300'}`}>{!isScheduling && <div className="w-2.5 h-2.5 bg-summo-primary rounded-full" />}</div><div><p className="font-bold text-sm text-gray-800 dark:text-gray-100">Padrão (Agora)</p><p className="text-xs text-gray-500">{settings.orderModes?.delivery?.minTime || 30}-{settings.orderModes?.delivery?.maxTime || 60} min</p></div></div><span className="text-xs font-bold text-green-600 bg-green-100 px-2 py-1 rounded">Rápido</span></label>
                    {settings.orderModes?.scheduling?.enabled && (<div className={`rounded-xl border-2 transition overflow-hidden ${isScheduling ? 'border-summo-primary bg-summo-bg/10' : 'border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800'}`}><label className="flex items-center justify-between p-4 cursor-pointer" onClick={() => { setIsScheduling(true); if (!scheduledTime && generateTimeSlots.length > 0) setScheduledTime(generateTimeSlots[0]); }}><div className="flex items-center gap-3"><div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${isScheduling ? 'border-summo-primary' : 'border-gray-300'}`}>{isScheduling && <div className="w-2.5 h-2.5 bg-summo-primary rounded-full" />}</div><div><p className="font-bold text-sm text-gray-800 dark:text-gray-100">Agendar</p><p className="text-xs text-gray-500">Escolha um horário</p></div></div></label>{isScheduling && (<div className="px-4 pb-4 animate-fade-in"><div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">{generateTimeSlots.map(time => (<button key={time} onClick={() => setScheduledTime(time)} className={`flex-shrink-0 px-3 py-2 rounded-lg text-xs font-bold border transition ${scheduledTime === time ? 'bg-summo-primary text-white border-summo-primary' : 'bg-white border-gray-200 text-gray-600'}`}>{time}</button>))}</div></div>)}</div>)}
                </div>
            </div>
        </div>
    );
    ...
    const renderPaymentStep = () => (
        <div className="flex-1 overflow-y-auto p-4 space-y-6 animate-slide-in-right">
            <h3 className="font-bold text-gray-800 dark:text-white text-lg">Pagamento pelo app</h3>
            <div className="space-y-3">{[{ id: 'Pix', label: 'Pix', icon: Smartphone, color: 'text-teal-600', sub: 'Aprovação imediata' }, { id: 'Cartão', label: 'Cartão na Entrega', icon: CreditCard, color: 'text-blue-600', sub: 'Levar maquininha' }, { id: 'Dinheiro', label: 'Dinheiro', icon: Banknote, color: 'text-green-600', sub: 'Pagamento na entrega' }].map((method) => (<label key={method.id} className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition shadow-sm hover:shadow-md ${paymentMethod === method.id ? 'border-summo-primary bg-summo-bg/10 ring-1 ring-summo-primary/20' : 'border-gray-100 bg-white dark:bg-gray-800 dark:border-gray-700'}`}><input type="radio" name="payment" className="hidden" checked={paymentMethod === method.id} onChange={() => setPaymentMethod(method.id)} /><div className={`p-3 rounded-full mr-4 bg-gray-50 dark:bg-gray-700 ${paymentMethod === method.id ? 'bg-white shadow-sm' : ''}`}><method.icon size={24} className={method.color} /></div><div className="flex-1"><span className="font-bold text-sm text-gray-800 dark:text-gray-100 block">{method.label}</span><span className="text-xs text-gray-500">{method.sub}</span></div><div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${paymentMethod === method.id ? 'border-summo-primary bg-summo-primary' : 'border-gray-300'}`}>{paymentMethod === method.id && <div className="w-2 h-2 rounded-full bg-white" />}</div></label>))}</div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700"><h4 className="text-xs font-bold text-gray-500 uppercase mb-3">Resumo Final</h4><div className="space-y-2 text-sm"><div className="flex justify-between text-gray-600 dark:text-gray-400"><span>Subtotal</span><span>R$ {cartTotal.toFixed(2)}</span></div>{isDelivery && <div className="flex justify-between text-gray-600 dark:text-gray-400"><span>Taxa de Entrega</span><span>R$ {deliveryFee.toFixed(2)}</span></div>}{discountValue > 0 && <div className="flex justify-between text-green-600 font-bold"><span>Desconto</span><span>- R$ {discountValue.toFixed(2)}</span></div>}<div className="flex justify-between font-bold text-lg text-gray-900 dark:text-white pt-2 border-t border-gray-200 dark:border-gray-600 mt-2"><span>Total a pagar</span><span>R$ {finalTotal.toFixed(2)}</span></div></div></div>
        </div>
    );

    return (
        <div className="fixed inset-0 z-[100] bg-white dark:bg-gray-900 flex flex-col h-full animate-slide-in-right">
            {renderHeader()}
            ...
            <div className="p-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 pb-safe shadow-[0_-4px_30px_rgba(0,0,0,0.08)] z-20">
                {step !== 'PAYMENT' ? (
                    <div className="flex justify-between items-center gap-4">
                        <div className="flex flex-col"><span className="text-[10px] text-gray-500 font-bold uppercase">Total</span><span className="text-xl font-bold text-gray-900 dark:text-white leading-none">R$ {finalTotal.toFixed(2)}</span></div>
