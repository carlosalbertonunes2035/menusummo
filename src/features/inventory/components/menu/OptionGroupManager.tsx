...
const OptionGroupManager: React.FC<OptionGroupManagerProps> = ({ isOpen, onClose, onLinkGroup, existingGroupIds, isEmbedded = false }) => {
    const { tenantId } = useApp();
    const { showToast } = useToast();
    const { optionGroups, saveOptionGroup, deleteOptionGroup } = useOptionGroupsQuery(tenantId);
    ...
    const handleSave = async () => {
        ...
        const isAdd = !isEditing.id;
        await saveOptionGroup(isEditing);
        showToast(`Grupo ${isAdd ? 'criado' : 'salvo'}!`, 'success');
        setIsEditing(null);
    };
    ...
    const Content = (
        <div onClick={e => e.stopPropagation()} className={`bg-white shadow-2xl flex flex-col overflow-hidden ${isEmbedded ? 'rounded-2xl w-full h-full border border-gray-100' : 'rounded-2xl w-full max-w-3xl h-[90vh]'}`}>
