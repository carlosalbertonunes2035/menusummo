    ...
    if (!settings.schedule || settings.schedule.length === 0) {
        return (
            <div className={cardClass}>
                <h4 className="font-bold text-slate-800 dark:text-slate-100 mb-4">Horário de Funcionamento</h4>
        ...
        <div className={cardClass}>
            ...
            <div className="space-y-3">
                {settings.schedule.map((day, index) => (
                    <div key={day.day} className="flex flex-wrap items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 gap-2">
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`font-bold text-sm ${day.isOpen ? 'text-slate-800 dark:text-slate-200' : 'text-slate-400'}`}>{day.day}</span>
                        </div>
                        <div className={`flex items-center gap-2 ${!day.isOpen ? 'opacity-30 pointer-events-none' : ''}`}>
                            <input type="time" value={day.openTime} onChange={e => onScheduleChange(index, 'openTime', e.target.value)} className="p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 text-sm" />
                            <span className="text-slate-400 text-xs">até</span>
                            <input type="time" value={day.closeTime} onChange={e => onScheduleChange(index, 'closeTime', e.target.value)} className="p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 text-sm" />
                        </div>
                    </div>
                ))}
            </div>
